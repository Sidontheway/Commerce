...
const OptionsPicker = ({
  ...
  return (
    <div className="flex flex-col gap-y-4">
      <div className="flex items-center justify-between border-t border-[#d8c9b8] px-1 pt-6">
        <span className="text-xs font-semibold uppercase tracking-[0.18em] text-[#a35b3e]">
          Filter by
        </span>
      </div>
      <Accordion.Root
        ...
        {options.map((option) => {
          ...
          return (
            <Accordion.Item
              key={option.id}
              value={option.id}
              className="overflow-hidden border-b border-[#e8ddd0]"
            >
              <Accordion.Header>
                <Accordion.Trigger className="flex w-full items-center justify-between py-3 text-left">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-[#40251f]">
                      {option.title || "Option"}
                    </span>
                    <span className="text-xs text-[#a35b3e]">
                      ({selectedCount})
                    </span>
                  </div>
                  <span
                  ...
                  {values.map((value) => {
                    ...
                    return (
                      <button
                        ...
                        className={clsx(
                          "border border-[#d8c9b8] bg-[#fffaf2] px-3 py-2 text-sm text-[#745d50] transition-colors duration-150 hover:border-[#a35b3e] hover:text-[#40251f]",
                          {
                            "border-[#a35b3e] bg-[#f3dbc0] text-[#40251f]":
                              isSelected,
                          }
                        )}
